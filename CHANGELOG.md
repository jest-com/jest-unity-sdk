...
- `JS_registerLifecycleCallbacks` in the JS bridge layer.
...
- `Payment.CancelSubscription(subscriptionSku)` — opens the platform cancellation confirmation dialog for a subscription. Returns a `CancelSubscriptionResult` with `Result` ("success", "cancel", or "error") and `Error` ("internal_error", "not_found", "not_active", or "guest_not_allowed").
- `Payment.CancelSubscriptionResult` DTO.
- `JS_cancelSubscription` in the JS bridge layer.
- `PaymentController.CancelSubscription()` / `CancelSubscription(string)` sample methods.
